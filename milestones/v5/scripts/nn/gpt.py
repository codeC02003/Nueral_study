...
class Config:
    def __init__(self, vocab_size=96, block_size=128, n_layer=4,
                 n_head=4, n_embd=128, dropout=0.0, use_mlp=True):
        ...
        # attention-only (use_mlp=False) is what makes the induction circuit's
        # ground truth exact - see PREREGISTRATION_V5.md
...
class Block(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.use_mlp = getattr(cfg, "use_mlp", True)
    ...
    def forward(self, x, keep=None):
        ...
        if not self.use_mlp:
            if keep is not None: keep["mlp"] = None
            return x
